...
export const architectureNarrativeText = 'LUNARA is not a single-page brochure with a login attached. It is a coordinated postpartum support system with three distinct surface areas — a public brand experience that builds trust before asking for engagement, a provider command center for managing an entire doula practice, and a client recovery workspace that turns fragmented postpartum care into a single continuous relationship. The backend is built on Express with TypeScript and follows a route → service → model separation. 20 route modules delegate to 29 service modules that encapsulate business logic for authentication, scheduling, messaging, content, documents, care plans, check-ins, file storage, email, push notifications, and caching. Passport.js manages three strategies: JWT for stateless API access, Local for email/password with bcrypt and lockout, and Google OAuth 2.0 for social login. The frontend uses React 18 on Vite with Tailwind CSS and code-splits every page with React.lazy. Role-based ProtectedRoute wrappers gate provider and client dashboards. State flows through AuthContext and ResourceContext, while an Axios client silently refreshes expired tokens, retries rate-limited requests with backoff, and broadcasts session-expiry events across browser tabs. Realtime messaging runs on Socket.IO with JWT verification in the handshake middleware, per-user room management, delivery acknowledgements, and a per-user message rate limiter. The frontend falls back to REST polling when the socket connection drops, so messages are never lost. The CI/CD pipeline consists of three GitHub Actions workflows: frontend-ci (ESLint, Jest, build, Playwright on Node 18/20), backend-ci (type-check, Jest, Docker build on main), and build (SonarQube analysis). Husky pre-commit hooks run lint-staged Prettier, ESLint, and TypeScript checks. Docker Compose orchestrates the full stack locally: MongoDB 7, the Node 20 backend with health checks, an Nginx-fronted frontend, and SonarQube Community with Postgres for static analysis. The deployment topology splits across Vercel (frontend hosting with instant rollbacks) and Render (backend with a render.yaml blueprint specifying health checks, environment secrets, and auto-deploy from main). MongoDB Atlas provides managed persistence. The Swagger UI at /api-docs offers an interactive API browser backed by OpenAPI 3.0 annotations across every route and model file.';

export const narrativePullQuote = '"Context follows the care relationship instead of being rebuilt at every interaction."';
