...
As feature work stabilized, the project shifted emphasis toward formal verification. Backend integration tests exercised real HTTP request/response cycles across authentication, appointment, messaging, check-in, care-plan, intake, and resource routes. Frontend tests validated component rendering, service-layer behavior, authentication flows, and protected-route guards. By the end of this phase the combined Jest suite reached **1044** automated tests (**891** frontend, **153** backend) across **118** suites, with **32** Playwright E2E tests, Jest statement coverage of **90.58%** on the backend and **63.35%** on the frontend (see `05_milestone_5.tex`), and a SonarQube aggregate coverage report (**81.9%** at last sprint capture). A SonarQube quality analysis workflow was integrated into the development process, and the project achieved "A" ratings across security, reliability, and maintainability dimensions. Findings from both automated testing and static analysis were fed back into iterative code fixes, strengthening the implementation before release.
